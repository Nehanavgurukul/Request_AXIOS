            ...
            async function fun1(){
                ...
                axios.get("http://saral.navgurukul.org/api/courses/"+id_array[index]+"/exercise/getBySlug?slug="+slug_id).then((Respdata) => {
                    ...
                    async function slugFun(){
                        ...
                        var t = true
                        while(t == true){
                            var user_choise = readlinSync.question("Enter the choise what you want to do:-")
                            if(user_choise == "up"){
                                upGet()
                            }else if(user_choise == "previous"){
                                if(slug_index == 0){
                                    console.log("There is no previous")
                                }else{
                                    console.log(slug_array[slug_index -1])
                                }
                            }else if(user_choise == "next"){
                                if((slug_array.length - 1) == slug_index){
                                    console.log("There is no next slug")
                                }else{
                                    console.log(slug_array[Number(slug_index) + 1])
                                }
                            }else{
                                if(user_choise == "stop"){
                                    t = false;
